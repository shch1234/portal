...
CREATE TABLE IF NOT EXISTS device_org_relation (
    ...
    unit_code       VARCHAR(100) NOT NULL COMMENT '组织单元编码（全局唯一）',
...
CREATE TABLE IF NOT EXISTS device_type_relation (
    id              BIGINT NOT NULL PRIMARY KEY COMMENT '主键ID（雪花算法）',
    type_code       VARCHAR(100) NOT NULL COMMENT '设备类型编码（字典 value，全局唯一）',
...
CREATE TABLE IF NOT EXISTS device_model (
    id                  BIGINT NOT NULL PRIMARY KEY COMMENT '主键ID（雪花算法）',
    model_code          VARCHAR(100) NOT NULL COMMENT '型号编码（全局唯一）',
...
CREATE TABLE IF NOT EXISTS device_info (
    id                  BIGINT NOT NULL PRIMARY KEY COMMENT '主键ID（雪花算法）',
    tb_device_id        CHAR(36) NOT NULL COMMENT 'ThingsBoard 设备ID（关联 ThingsBoard device.id，可通过 ThingsBoard API 查询获取租户信息）',
    device_code         VARCHAR(100) NOT NULL COMMENT '设备编号（威力编号，全局唯一）',
...
CREATE INDEX idx_webhook_inbox_status_received ON webhook_inbox (status, received_time);
